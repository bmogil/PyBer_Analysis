...
3. Collecting the average wait time per customer per city type. I believe this would open up a whole new window of observation for the business because this metric could be compared with total revenue amongst other variables like average ride fare, over different time periods to potentially determine if this is  impacting any seasonal demand patterns, and other potentially noteworthy correlations. Moreover, if average wait time and total revenue are negatively correlated, (maybe because when wait times go up, less people use PyBer) Pyber could then infuse more drivers in this area to decrease the average wait time. Also, if demand seasonality patterns are consistently indicated in the year-over data, PyBer could more optimally allocate driver resources to maximize profitability.  
